...
> `index_cancel(job_id=...)` to stop it. A repo already in the registry re-indexes
> under its own graph key; a NEW path requires an explicit `graph_key` (keys owned
> by another repo are rejected). Or from the CLI `cih-engine analyze <repo>`.
...
rmcp 0.7). **cih-server is a layered hexagonal monolith** — `bootstrap.rs` is the
composition root; `transport/mcp` (tool routers, DTOs, resources) and
`transport/http` are thin adapters; `application/` owns validated commands and
typed outputs; `domain/` is protocol-independent; `ports/` defines external
boundaries; `infrastructure/` implements them (caches, registry, graph store,
git, index jobs). **Dependency rule:** `application` and `domain` must not import
`rmcp`, `axum`, `crate::infrastructure`, or `crate::transport` — enforced by
`crates/cih-server/tests/architecture_boundaries.rs`, which fails the build on a
violation. See `docs/ARCHITECTURE.md` (layers) and `docs/DEVELOPERS.md` (how to
add a capability). The graph store is pluggable:
...
together.
Note: `transport/http/browser.rs` + `layout.rs` in cih-server are the live
graph-browser UI served at `/graph` (tested by `tests/browser.rs`) — not dead
code. Both binaries are thin shims: server logic lives in `cih_server`
(entry point `bootstrap::run`), engine modules in `cih_engine` (used by
`main.rs` via `use cih_engine::…`).
...
This project is indexed by GitNexus as **yummy-cih** (15160 symbols, 35761 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
