...
mod tests {
    ...
    fn duplicate_edges_are_deduplicated() {
        ...
        let layout = compute(overview);
        // Two unique edges: (A→B, CALLS) and (A→B, IMPORTS). The duplicate CALLS is dropped.
    ...
    fn unknown_kind_gets_default_size_and_color() {
        let overview = GraphOverview {
            nodes: vec![
                node("Other:widget", NodeKind::Other, "ext/widget.py", 5),
            ],
            edges: Vec::new(),
            total_nodes: 1,
            total_edges: 0,
        ...
        // Default base size is 3.2 for Other kind, plus log(degree+1)*0.75
        assert!(n.size > 3.0 && n.size < 20.0, "size {} out of expected range", n.size);
        // degree=5 → stellar color "#ffc070" (warm yellow)
    ...
    fn large_degree_does_not_overflow() {
        let overview = GraphOverview {
            nodes: vec![
                node("Method:Hub#dispatch/0", NodeKind::Method, "src/hub.rs", 999_999),
            ],
            edges: Vec::new(),
            total_nodes: 1,
            total_edges: 0,
