...
use crate::application::admin::resolve_patterns::{
    AddResolvePatternCommand, ListResolvePatternsCommand,
};
...
impl CihServer {
    ...
    async fn add_resolve_pattern(
        ...
        let output = self
            .resolve_pattern_service()
            .add(AddResolvePatternCommand {
                repo: args.repo,
                kind: args.kind,
                annotation: args.annotation,
                path_attr: args.path_attr,
                method: args.method,
                method_attr: args.method_attr,
                class_prefix_annotation: args.class_prefix_annotation,
    ...
    async fn list_resolve_patterns(
        ...
        let output = self
            .resolve_pattern_service()
            .list(ListResolvePatternsCommand { repo: args.repo })
            .map_err(app_error_to_mcp)?;
        json_result(&output)
    }
}
