...
pub fn auto_detect_architecture(repo_map: &RepoMap) -> ArchitectureHint {
    let total = repo_map.total_source_files;
...
pub struct RepoMap {
    ...
    /// Approx LOC across all source files (newline count — no parsing).
    pub total_source_loc: u64,
...
pub struct ModuleInfo {
    ...
    pub rel_path: String,
    /// `pom.xml` / `build.gradle[.kts]` / `package.json` / `pyproject.toml` if present.
    pub build_file: Option<String>,
    /// Total source files in this module (all languages).
    pub source_files: u64,
    /// Approx LOC across all source files in this module.
    pub source_loc: u64,
    /// Top-level packages/namespaces found in the module (best-effort).
    pub packages: Vec<String>,
    /// Sibling modules this one depends on (best-effort from build files).
    pub depends_on: Vec<String>,
    /// Detected framework/runtime identifiers, e.g. `["spring", "nestjs"]`.
    #[serde(default)]
    pub frameworks: Vec<String>,
    /// Per-language source file counts within this module.
    #[serde(default)]
    pub per_language: BTreeMap<String, u64>,
}
