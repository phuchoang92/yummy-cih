...
pub fn make_parser() -> Parser {
    ...
    p.set_language(&language()).expect("Rust parser must load");
    p
}
...
impl LanguageProvider for RustProvider {
    ...
    fn scan_file(&self, _rel: &str, src: &str) -> anyhow::Result<SourceScan> {
        ...
        if src.contains("actix_web") || src.contains("actix-web") {
            frameworks.insert("actix-web".into());
        }
        if src.contains("axum::") || src.contains("use axum") {
            frameworks.insert("axum".into());
        }
        if src.contains("rocket::") || src.contains("use rocket") {
            frameworks.insert("rocket".into());
        }
