...
pub const RB_SCOPE_QUERY: &str = include_str!("query.scm");

static QUERY: Lazy<Query> =
    Lazy::new(|| Query::new(&language(), RB_SCOPE_QUERY).expect("Ruby scope query must compile"));
...
pub fn make_parser() -> Parser {
    ...
    p.set_language(&language()).expect("Ruby parser must load");
    p
}
...
impl LanguageProvider for RubyProvider {
    ...
    fn language_id(&self) -> &'static str { "ruby" }
    ...
    fn parse_file(&self, rel: &str, src: &str) -> anyhow::Result<cih_core::ParsedUnit> {
        parse::parse_ruby_file(rel, src)
    }

    fn scan_file(&self, _rel: &str, src: &str) -> anyhow::Result<SourceScan> {
        let loc = src.bytes().filter(|b| *b == b'\n').count() as u64;
        let mut frameworks = BTreeSet::new();
        if src.contains("Rails.application") || src.contains("ActionController") {
            frameworks.insert("rails".into());
        }
        if src.contains("Sinatra::Base") || src.contains("require 'sinatra'") {
            frameworks.insert("sinatra".into());
        }
