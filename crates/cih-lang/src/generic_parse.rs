...
use cih_core::{
    function_id, type_id, Edge, EdgeKind, Node, NodeId, NodeKind, ParsedFile, ParsedUnit, Range,
    RawImport, RefKind, ReferenceSite, SymbolDef,
};
...
pub fn emit_method(
    ...
    nodes.push(Node {
        ...
        kind: NodeKind::Method,
        name: name.to_string(),
        qualified_name: Some(format!("{owner_fqcn}.{name}")),
        file: rel.to_string(),
        range,
        props: None,
    });
