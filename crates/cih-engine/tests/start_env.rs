...
fn temp_dir(suffix: &str) -> PathBuf {
    ...
    let _ = std::fs::remove_dir_all(&path); // Clean up if exists
...
fn start_load_env_returns_lines() {
    ...
    std::fs::write(&path, "REPO_PATH=/repo\nREPO_NAME=test\n").expect("write failed");
...
fn start_write_new_env_creates_file() {
    ...
    let written = std::fs::read_to_string(&path).expect("failed to read .env");
...
fn start_write_backup_created() {
    ...
    std::fs::write(&path, "REPO_PATH=/old\n").expect("initial write failed");
    ...
    let backup_files: Vec<_> = std::fs::read_dir(&temp)
    ...
    let backup_content =
        std::fs::read_to_string(backup_files[0].path()).expect("failed to read backup");
