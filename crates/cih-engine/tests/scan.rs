...
fn scan_mixed_repo_java_ts_python() {
    ...
    let java_mod = repo_map.modules.iter().find(|m| m.name == "java-app").unwrap();
    assert_eq!(java_mod.source_files, 1);
    assert!(java_mod.frameworks.contains(&"spring".to_string()));

    // Node Module
    let node_mod = repo_map.modules.iter().find(|m| m.name == "frontend-web").unwrap();
    assert_eq!(node_mod.source_files, 1);
    assert!(node_mod.frameworks.contains(&"nestjs".to_string()));

    // Python Module
    let py_mod = repo_map.modules.iter().find(|m| m.name == "python-backend").unwrap();
...
fn scan_node_repo_package_json() {
    ...
    let root_mod = repo_map.modules.iter().find(|m| m.name == "root-project").unwrap();
    assert_eq!(root_mod.source_files, 1);
    assert!(root_mod.depends_on.contains(&"api-server".to_string()));

    let api_mod = repo_map.modules.iter().find(|m| m.name == "api-server").unwrap();
...
fn scan_python_repo_pyproject_toml() {
    ...
    let main_mod = repo_map.modules.iter().find(|m| m.name == "my-fastapi-app").unwrap();
