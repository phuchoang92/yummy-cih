...
use cih_graph_store::{
    default_projection_edge_kinds, stored_edge_token, BackendReadiness, CallSiteArgs,
    CommunityEdge, CommunityInfo, ContextFilter, ContextPage, ContextSection, DbEffect, Direction,
        ...
        match query.scope {
            ProjectionScope::Repository => {
                ...
                let mut rows = self
                    .rows(&format!(
                        "MATCH (c:Symbol) WHERE c.kind = 'Community' \
                         RETURN c.id, c.name, coalesce(c.symbolCount, 0) AS members \
                         ORDER BY members DESC, c.id ASC LIMIT {probe}"
                    ))
                    .await?;
                ...
                let mut nodes = rows
                    ...
                    .map(|row| GraphProjectionNode {
                        id: NodeId::new(row[0].clone()),
                        kind: NodeKind::Community,
                        name: row[1].clone(),
                        role: ProjectionNodeRole::Aggregate,
                        member_count: row.get(2).and_then(|v| v.parse().ok()).unwrap_or(0),
                        degree: 0,
                        expandable: true,
                    })
                    .collect::<Vec<_>>();
                let mut total_nodes = self
                    .rows("MATCH (c:Symbol) WHERE c.kind = 'Community' RETURN count(c)")
                    .await?
                    .first()
                    .and_then(|row| row.first())
                ...
                if nodes.is_empty() {
                    ...
                    total_nodes = self
                        .rows("MATCH (f:Symbol) WHERE f.kind = 'Folder' AND NOT f.file CONTAINS '/' RETURN count(f)")
                        .await?
                        .first()
                        .and_then(|row| row.first())
                        .and_then(|value| value.parse().ok())
                    ...
                    let edge_rows = self
                        .rows(&format!(
                            "MATCH (a:Symbol)-[:MEMBER_OF]->(ca:Symbol), \
                                   (b:Symbol)-[:MEMBER_OF]->(cb:Symbol), (a)-[r]->(b) \
                             WHERE ca.id IN [{ids}] AND cb.id IN [{ids}] AND ca.id <> cb.id \
                               AND type(r) IN [{kind_list}] \
                             RETURN ca.id, cb.id, type(r), count(*) AS weight \
                             ORDER BY weight DESC, ca.id ASC, cb.id ASC LIMIT {edge_probe}"
                        ))
                        .await?;
                    for row in edge_rows.iter().filter(|row| row.len() >= 4) {
                        *edge_counts
                            .entry((
                                NodeId::new(row[0].clone()),
                                NodeId::new(row[1].clone()),
                                edge_from_label(&row[2]),
                            ))
                            .or_insert(0) += row[3].parse::<u64>().unwrap_or(0);
                    }
            ...
            ProjectionScope::Community => {
                ...
                let mut file_rows = self
                    .rows(&format!(
                        "MATCH (n:Symbol)-[:MEMBER_OF]->(c:Symbol) \
                         ...
                         RETURN n.file, count(n) AS members \
                         ORDER BY members DESC, n.file ASC LIMIT {probe}"
                    ))
                    .await?;
                ...
                let total_files: u64 = self
                    .rows(&format!(
                        "MATCH (n:Symbol)-[:MEMBER_OF]->(c:Symbol) \
                         WHERE c.id = {parent_lit} AND n.file <> '' RETURN count(DISTINCT n.file)"
                    ))
                    .await?
                    .first()
                    .and_then(|row| row.first())
                ...
                let mut nodes = file_rows
                    ...
                    .filter(|row| row.len() >= 2)
                    .map(|row| GraphProjectionNode {
                        id: NodeId::new(format!("File:{}", row[0])),
                        kind: NodeKind::File,
                        name: path_name(&row[0]),
                        role: ProjectionNodeRole::Aggregate,
                        member_count: row[1].parse().unwrap_or(0),
                        degree: row[1].parse().unwrap_or(0),
                        expandable: true,
                    })
                    .collect::<Vec<_>>();
                ...
                if !selected_files.is_empty() {
                        ...
                        .rows(&format!(
                            "MATCH (a:Symbol)-[:MEMBER_OF]->(c:Symbol)<-[:MEMBER_OF]-(b:Symbol), \
                             ...
                             WHERE c.id = {parent_lit} AND a.file IN [{file_list}] \
                               ...
                               AND type(r) IN [{kind_list}] \
                             RETURN a.file, b.file, type(r), count(*) AS weight \
                             ORDER BY weight DESC, a.file ASC, b.file ASC LIMIT {edge_probe}"
                        ))
                        .await?;
                    for row in internal.iter().filter(|row| row.len() >= 4) {
                        *edge_counts
                            .entry((
                                NodeId::new(format!("File:{}", row[0])),
                                NodeId::new(format!("File:{}", row[1])),
                                edge_from_label(&row[2]),
                            ))
                            .or_insert(0) += row[3].parse::<u64>().unwrap_or(0);
                    }
                        ...
                        .rows(&format!(
                            "MATCH (a:Symbol)-[:MEMBER_OF]->(c:Symbol), \
                             ...
                             WHERE c.id = {parent_lit} AND other.id <> c.id \
                               AND a.file IN [{file_list}] AND type(r) IN [{kind_list}] \
                             RETURN a.file, other.id, other.name, type(r), count(*) AS weight \
                             ORDER BY weight DESC, a.file ASC, other.id ASC LIMIT {boundary_probe}"
                        ))
                        .await?;
                    ...
                    for row in outgoing.iter().take(boundary_budget) {
                        ...
                        let boundary = NodeId::new(row[1].clone());
                                ...
                                < boundary_budget
                        {
                            nodes.push(GraphProjectionNode {
                                ...
                                member_count: 0,
                                degree: row[4].parse().unwrap_or(0),
                                expandable: true,
                            });
                        }
                        *edge_counts
                            .entry((
                                NodeId::new(format!("File:{}", row[0])),
                                boundary,
                                edge_from_label(&row[3]),
                            ))
                            .or_insert(0) += row[4].parse::<u64>().unwrap_or(0);
                    }
                }
            ...
            ProjectionScope::File => {
                ...
                let file_lit = cstr(&parent_node.file);
                let probe = query.node_limit.saturating_add(1);
                let columns = node_columns("n");
                let mut symbol_rows = self
                    .rows(&format!(
                        "MATCH (n:Symbol) WHERE n.file = {file_lit} \
                           AND n.kind NOT IN ['File','Folder','Community','Process'] \
                ...
                let total_symbols = self
                    .rows(&format!(
                        "MATCH (n:Symbol) WHERE n.file = {file_lit} \
                         AND n.kind NOT IN ['File','Folder','Community','Process'] RETURN count(n)"
                    ))
                    .await?
                    .first()
                    .and_then(|row| row.first())
                ...
                let nodes = symbol_rows
                    ...
                    .map(|row| {
                        ...
                        GraphProjectionNode {
                            ...
                            member_count: 1,
                            degree: row[7].parse().unwrap_or(0),
                            expandable: true,
                        }
                    })
                    .collect::<Vec<_>>();
                    ...
                    let internal = self
                        .rows(&format!(
                            "MATCH (a:Symbol)-[r]->(b:Symbol) WHERE a.id IN [{ids}] \
                               AND b.id IN [{ids}] AND type(r) IN [{kind_list}] \
                             RETURN a.id, b.id, type(r), count(*) AS weight \
                             ORDER BY weight DESC, a.id ASC, b.id ASC LIMIT {edge_probe}"
                        ))
                        .await?;
                    for row in internal.iter().filter(|row| row.len() >= 4) {
                        *edge_counts
