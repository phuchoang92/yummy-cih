...
mod tests {
    use super::*;
    use cih_core::{SymbolDef, TypeBinding};
    ...
    fn extract_xml_attr_respects_boundaries() {
        ...
        // `myclass` should not match `class`.
        assert_eq!(extract_xml_attr("<bean myclass=\"x\"/>", "class"), None);
    }

    /// End-to-end over a temp repo: a Spring bean class injected as a field should
    /// produce a CALLS edge from the consuming class to the bean class.
    #[test]
