...
mod tests {
    ...
    impl Embedder for MockEmbedder {
        fn embed(&self, texts: &[String]) -> anyhow::Result<Vec<Vec<f32>>> {
            Ok(texts.iter().map(|t| {
                // Very simple: hash first char to a unit vector dimension
    ...
    fn embed_strategy_assigns_residuals_above_threshold() {
        ...
        // 'P' → index 80 in ASCII mod 4 = 0; 'S' → index 83 mod 4 = 3
        // payment nodes: name starts with 'P' → dim 0
        // residual: name starts with 'P' → dim 0 → similar to payment centroid
        let nodes = vec![
            make_node("Class:com.example.PaymentService", "PaymentService", "payment-service/src/main/java/com/example/PaymentService.java"),
            make_node("Class:com.example.PaymentRepo", "PaymentRepo", "payment-service/src/main/java/com/example/PaymentRepo.java"),
            make_node("Class:com.example.custom.PayProcessor", "PayProcessor", "custom-impl/src/main/java/com/example/PayProcessor.java"),
        ];

        let prior = vec![
            FeatureGroupEntry { id: "feature:payment".into(), name: "payment".into(), node_id: "Class:com.example.PaymentService".into(), strategy: "package".into(), confidence: 1.0, pinned: false, evidence: String::new(), node_content_hash: 0 },
            FeatureGroupEntry { id: "feature:payment".into(), name: "payment".into(), node_id: "Class:com.example.PaymentRepo".into(), strategy: "package".into(), confidence: 1.0, pinned: false, evidence: String::new(), node_content_hash: 0 },
            FeatureGroupEntry { id: "feature:shared".into(), name: "shared".into(), node_id: "Class:com.example.custom.PayProcessor".into(), strategy: "package".into(), confidence: 1.0, pinned: false, evidence: String::new(), node_content_hash: 0 },
        ];

        let cfg = EmbedConfig {
            similarity_threshold: 0.99, // very high — only exact matches
            ..EmbedConfig::default()
        };
        ...
        // PayProcessor starts with 'P' same as PaymentService → same bucket → similarity 1.0 → assigned
