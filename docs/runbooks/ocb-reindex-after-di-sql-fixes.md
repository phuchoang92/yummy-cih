...
(`docs/plans/ocb-mcp-improvements.md`). Parse cache schema bumped **25 → 26**;
search sidecar schema fingerprint changed.
...
- **`reaches` answers the AUDIT_LOG question directly**:
  `reaches(from="confirmChangePassword", to="AUDIT_LOG")` returns evidence paths
  with per-edge confidence; bare table names fall back to the `DbTable:` id.
- **`status` shows real route counts** right after analyze (no discover needed).
  Entries indexed by older builds still show `routes: 0` until re-analyzed.
...
- **Single-file `grep_files` is O(1)**: a metacharacter-free glob stats the file
  ...
  to their subtree. Default grep concurrency is now 2
